...
class Build(object):
    ...
    def __init__(self, url, job, number, data):
    ...
    def console_output(self):
        ...
        """
        return requests.get(urljoin(self.url, "/job/{}/{}/consoleText".format(self.job, self.number))).text
...
class Jenkins(object):
    ...
    def builds(self, job, tree):
        ...
        for build in requests.get(url, params, auth=self._auth).json()['builds']:
            yield Build(self._url, job, build['number'], build)
